...
def initialize_figure(bgcolor=(0.16, 0.16, 0.24)):
    ...
    mlab.plot3d([0, axes[0, 0]], [0, axes[0, 1]], [0, axes[0, 2]],
                color=(1, 0, 0), tube_radius=0.2, figure=fig)
...
def draw_lidar(pc, color=None, fig=None, bgcolor=(0.16, 0.16, 0.24), pts_scale=1, pts_mode='point', pts_color=None):
    ''' Draw lidar points
    Args:
    ...
    if fig is None:
        fig = mlab.figure(figure=None, bgcolor=bgcolor,
                          fgcolor=None, engine=None, size=(1600, 1000))
    if color is None:
        color = np.abs(pc[:, 0]) * 2 + 40
    mlab.points3d(pc[:, 0], pc[:, 1], pc[:, 2], color, color=pts_color,
                  mode=pts_mode, colormap='Blues', scale_factor=pts_scale*10, figure=fig)
...
def draw_heading_arrow(gt_boxes3d, fig, color=(1, 1, 1)):
    # Draw heading arrow
    num = len(gt_boxes3d)
    for n in range(num):
        b = gt_boxes3d[n]
