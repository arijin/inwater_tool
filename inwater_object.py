...
class inwater_object(object):
    ...
    def get_lidar(self, idx):
        ...
        scan = np.fromfile(lidar_filename, dtype=np.float32)  # read bin file.
        # print(scan.shape)
...
#     fov_inds = fov_inds & (pc_velo[:,0]>clip_distance)  # 俯瞰图下距离自身2.0m开外。
...
def dataset_viz():
    ...
    for data_idx in range(3):  # len(dataset)
        ...
        show_image_with_boxes(img, objects, calib, annotator, True)
        raw_input()
